...
export function getJoinPotProof(joineeAddress: Address, participants: Address[]) {
    ...
    // Create leaf nodes
    const leaves = participants.map(addr =>
        keccak256(toBytes(addr))
    );
    ...
    const isValid = tree.verify(proof, Buffer.from(joineeLeaf.slice(2), 'hex'), Buffer.from(root.slice(2), 'hex'));
