...
export default function PotList() {
  ...
  const [selectedPeriod, setSelectedPeriod] = useState<'all' | 'daily' | 'weekly' | 'biweekly' | 'monthly'>('all');
  ...
  useEffect(() => {
    ...
    (async () => {
      const _logs = await getPotCreatedLogs() as unknown as Array<{ address: Address; args: { potId: bigint; creator: Address }; blockNumber: number; transactionHash: string }>;
  ...
  const filteredPots: TPotObject[] = selectedPeriod === 'all' ? pots : pots.filter((pot) => pot.period === periodSecondsMap[selectedPeriod]);
    ...
    <div>
      ...
      <div className='max-w-min'>
        ...
        {['all', 'daily', 'weekly', 'monthly'].map((tab) => (
          <GradientButton2
            key={tab}
            onClick={() => { if (selectedPeriod !== tab) setSelectedPeriod(tab as typeof selectedPeriod); }}
      ...
      {!loading && filteredPots.length === 0 ? (
        <div className="text-center py-10 rounded-xl">
          No pots available.<br />Be the first to create one!
        </div>
      ) : (
        <div className="grid gap-[22px] md:grid-cols-2 lg:grid-cols-3">
          {filteredPots.map((pot: TPotObject) => <PotCard key={pot.id} pot={pot} isJoining={joiningPotId !== null} joiningPotId={joiningPotId} handleJoinPot={handleJoinPot} />)}
        </div>
      )}
      {loading ? <div className='mt-4 w-full flex justify-center'><Loader2 className='animate-spin' color='#7C65C1' size={32}/></div> : null}
    </div>
  );
}

export function PotCard({ pot, isJoining, joiningPotId, handleJoinPot }: { pot: TPotObject; isJoining: boolean; joiningPotId: bigint | null; handleJoinPot: (pot: TPotObject) => void }) {
  ...
  <GradientCard key={pot.id}>
   ...
   <GradientButton
      ...
      className="w-full"
    >
      {isConnecting ? 'Connecting' : !isConnected ? 'Connect' : isJoined ? 'Joined' : isJoining && joiningPotId === pot.id ? 'Joining...' : 'Join Pot'}
    </GradientButton>
    </Link>
  </GradientCard>)
}