...
export default function CreatePotPage() {
  ...
  const { allowance, isLoadingAllowance, refetchAllowance, approveTokensAsync } = useApproveTokens()
  ...
  const disabled = !isConnected || initialLoading || isLoading || !amount || !name || Number.parseFloat(amount) <= 0;
  ...
  const createPot = async (): Promise<bigint> => {
    try {
      ...
      const potCreatedEvent = receipt.logs.find(log => log.topics[0] === PotCreatedEventSignatureHash);
      if (!potCreatedEvent) {
      ...
      <div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
           ...
           : <GradientButton 
            onClick={(e) => { e.preventDefault(); ensureConnection(); }}
            type="button" 
            className="w-full"
          >
            Connect
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-center">
              <div className="py-4">
                ...
                <p className="mb-6">
                  Share with friends to start saving together. The more people that join, the more everyone saves!
                </p>
                {hash && <div>Transaction Hash: <Link href={`https://sepolia.basescan.org/tx/${hash}`}>{formatAddress(hash)}</Link></div>}
              </div>
            </DialogDescription>
          </DialogHeader>
