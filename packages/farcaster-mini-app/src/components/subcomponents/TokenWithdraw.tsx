...
const TokenWithdraw: React.FC<TokenWithdrawProps> = ({ address, isMyAddress }) => {
  ...
  const { isLoading: isLoadingPotluck } = usePotluck();
  const {
  ...
  const onClick = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (!validate()) return;
    if (!disabled && token) {
      handleWithdraw(BigInt(parseUnits(amount, token.decimals)));
    }
  };
  // TODO: take into account gas fee for dataNativeBalance

  const hasBalance =
    withdrawBalance !== undefined && withdrawBalance > BigInt(0);
  const hasNoBalance =
    withdrawBalance !== undefined && withdrawBalance === BigInt(0);
              ...
              >
                <div>
                  ...
                  <p className="pb-1 text-3xl font-bold">
                    {Number(
                      formatUnits(withdrawBalance ?? BigInt(0), token.decimals)
                    )}{" "}
                    {token.symbol}
                  </p>
                </div>
              ...
              <label htmlFor="enrty-amount" className="block mb-2.5">
                ...
                <span
                  className={`font-medium text-xs text-red-500 ${
                    showError("amount") ? "visible" : "hidden"
                  }`}
                >{`${errors.amount}`}</span>
              </label>
              <Input
